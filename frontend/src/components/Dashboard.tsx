...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  const lineData = [...sessions].reverse().filter(s => s.ats_score > 0).map(s => ({ date: s.date, score: s.ats_score }));
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         <div className="glass-card p-10 rounded-[3rem] border-white/10 flex flex-col items-center justify-center text-center group">
            <div className="relative w-48 h-48 flex items-center justify-center mb-6">
               <svg className="w-full h-full transform -rotate-90 scale-110">
                  ...
                  <circle cx="96" cy="96" r="88" stroke={healthColor} strokeWidth="12" fill="transparent" strokeDasharray={552.9} strokeDashoffset={552.9 - (552.9 * healthScore) / 100} />
               </svg>
               <span className="absolute text-6xl font-black text-white">{healthScore || '--'}</span>
            </div>
            <p className="text-sm font-black uppercase tracking-widest" style={{ color: healthColor }}>
         ...
         <div className="glass-card p-10 rounded-[3rem] border-white/10 flex flex-col justify-between">
            <h3 className="text-xs font-black text-gray-500 uppercase flex items-center gap-2"><Target size={14} /> Latest ATS</h3>
            {atsResult ? (
              <div className="text-6xl font-black text-white">{atsResult.ats_score}<span className="text-xl opacity-30">/100</span></div>
            ) : <p className="text-gray-600 text-sm font-bold py-10 uppercase italic">Engine standby...</p>}
            <button onClick={() => onNavigate('ats')} className="w-full py-4 bg-white/5 text-white rounded-2xl text-[10px] font-black uppercase flex items-center justify-center gap-2 mt-6">View Full Report <ChevronRight size={12} /></button>
         </div>

         <div className="glass-card p-10 rounded-[3rem] border-white/10 flex flex-col justify-between">
            <h3 className="text-xs font-black text-gray-500 uppercase flex items-center gap-2"><Activity size={14} /> Latest Interview</h3>
            {interviewReport ? (
              <div className="text-6xl font-black text-white">{interviewReport.overall_score}<span className="text-xl opacity-30">/100</span></div>
            ) : <p className="text-gray-600 text-sm font-bold py-10 uppercase italic">Simulation required...</p>}
            <button onClick={() => onNavigate('interview')} className="w-full py-4 bg-white/5 text-white rounded-2xl text-[10px] font-black uppercase flex items-center justify-center gap-2 mt-6">Retake Session <ChevronRight size={12} /></button>
         </div>
      </div>
